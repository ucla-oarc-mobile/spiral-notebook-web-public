...
import {
  ...
  gateCheckSharedArtifact,
  removeUnsafeSharedArtifact,
} from 'store/shared-artifacts'
...
class EditSharedArtifact extends React.Component {
  // Submission types.
  ...
  componentDidMount(props) {
    this.props.fetchSharedArtifact(this.state.artifactId)
    window.scrollTo(0, 0)
  }
  ...
  render() {
    ...
    if (!this.props.sharedArtifacts[this.state.artifactId]) {
      return this.renderLoadingScreen()
    }
      ...
      <div className="eq-page">
        <div role="main" id="main-content" className="edit-artifact">
          ...
          <form onSubmit={this.submit}>
            <H1 className="eq-title-block">Edit Shared Artifact</H1>
            {this.renderMessages()}
  ...
  getArtifact() {
    return this.props.sharedArtifacts[this.state.artifactId]
  }
  ...
  renderUploads() {
    let url = config.api.baseUrl + '/shared-portfolios/'
    url += this.getArtifact().sharedPortfolio.id + '/shared-artifacts/'
   ...
   */
  renderDeleteAction() {
    // Only allow shared artifacts without reactions/comments to be deleteable.
  ...
  renderEditWarning() {
    const unsafe = this.props.sharedArtifacts.unsafe
    ...
    const content = (
      <div>
        <h2>
          {unsafe.warning + ' '}
          Are you sure you want to overwrite their changes?
        </h2>
        ...
        <div className="actions">
          ...
          <Button
            ...
            onClick={() => {
              this.props.editSharedArtifact(this.state.portfolioId, unsafe)
            }}
          >
            YES
          </Button>
    ...
    return (
      <ModalPopover
        ...
        content={content}
        defaultIsOpen
        onOpened={(element) => element.style.opacity = 1}
        onClosed={this.props.removeUnsafeSharedArtifact}
      >
        &nbsp;
      </ModalPopover>
    )
  ...
  renderResponses() {
    ...
    Object.keys(responses).forEach(key => {
      if (responses[key].bonus) {
        bonusResponses.push(this.renderResponse(responses[key]))
      } else {
        regularResponses.push(this.renderResponse(responses[key]))
      }
    })
  ...
  submit = (event) => {
    ...
    if (formErrors) {
      ...
      this.props.gateCheckSharedArtifact(
        artifact.sharedPortfolio.id,
        readyArtifact,
        artifact.updated_at,
      )
    }
    ...
    return <div>
      ...
      <div className="actions">
        ...
        <Button
          className="bp3-button bp3-button bp3-minimal eq-button eq-button-inverted"
          onClick={() => {
            this.props.deleteSharedArtifact(this.getArtifact())
            this.setState({
              redirect: '/shared-portfolios/' + this.state.portfolioId
            })
          }}
        >YES</Button>
      </div>
...
const mapDispatchToProps = dispatch => ({
  ...
  gateCheckSharedArtifact: (sharedPortfolioId, artifact, updatedAt) => dispatch(
    gateCheckSharedArtifact(sharedPortfolioId, artifact, updatedAt)
  ),
  editSharedArtifact: (portfolioId, artifact) => dispatch(
    editSharedArtifact(portfolioId, artifact)
  ),
  fetchSharedArtifact: (id) => dispatch(fetchSharedArtifact(id)),
  removeUnsafeSharedArtifact: () => dispatch(removeUnsafeSharedArtifact()),
  setRedirect: path => dispatch(setRedirect(path)),
})

export default connect(mapStateToProps, mapDispatchToProps)(EditSharedArtifact)
